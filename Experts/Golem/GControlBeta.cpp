...
#define LINEAR_ACC 			400.0             
#define ANGULAR_ACC 			50.0
...
void GControl::SetSpeed(vector iSpeed, float imyh)
{
...
//       cout <<"\nmi e' stata impostata la pos (" <<dX<<","<<dY<<","<<dH<<")";
       myActualX=dX; myActualY=dY; myActualH=dH; 
       PosSetted = true;		
}
...
{
       return Point(myNextX,myNextY,myNextH);
}
...
void GControl::GetSpeed(vector &iSpeed, float &imyh)
{
	iSpeed=vector(myActualSpeed_myX, myActualSpeed_myY, Cartesian);
	imyh=myActualJog;
}
	
int GControl::GetJog()
{
	 return nearestInt(myActualJog);
}
